...
exports.handler = async function handler(event) {
  try {
    ...
    if (!user) {
      return badRequest('Bu email bilan account topilmadi. Avval ro‘yxatdan o‘ting yoki Google orqali kiring.');
    }

    if (!(user.authMethods || []).includes('email') || !user.passwordHash || !user.passwordSalt) {
      return badRequest(
        'Bu email uchun parol orqali kirish yoqilmagan. Shu email bilan Google orqali kiring yoki yangi email-parol bilan ro‘yxatdan o‘ting.'
      );
    }
